...
class Edit:
    ...
    def POST(self, id_evento, **k):
        if app.session.loggedin is True: # validate if the user is logged
            ...
            session_privilege = app.session.privilege # get the session_privilege
            if session_privilege == 0: # admin user
                return self.POST_EDIT(id_evento) # call POST_EDIT function
            elif session_privilege == 1: # guess user
                raise config.web.seeother('/guess') # render guess.html
        else: # the user dont have logged
            raise config.web.seeother('/login') # render login.html   
    ...
    @staticmethod
    def GET_EDIT(id_evento, **k):
        message = None # Error message
        id_evento = config.check_secure_val(str(id_evento)) # HMAC id_cliente validate
        result = config.model.get_clientes(int(id_evento)) # search for the id_cliente
    ...
    def POST_EDIT(id_evento, **k):
        ...
        if result == None: # Error on udpate data
            id_evento = config.check_secure_val(str(id_evento)) # validate HMAC id_cliente
            result = config.model.get_clientes(int(id_evento)) # search for id_cliente data
